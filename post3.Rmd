...
The original dataset includes __ rows and __ columns.
To make the prediction more meaningful, I have selected rows and columns.
For rows, I only chose rows of which the zipcode is 98006. This dataset only contains variables about features of house itself (such as #of room, area of specific place in the house etc..). Since house price is also subject to change according to external traits such as population of a city and development of public transportation and so on. In this sense, house price cannot be compared between san francisco and ohio. Hence, to remove the effect of location, I randomly selected just one zipcode 98006.
For columns, I excluded unnecessary, irrelevant variables with house price.

2. Overview of analysis techniques
Stepwise selection
equation here: Y=beta 0+B1X1+..
...
Since penalty of BIC is larger, it results in producing a more parsimonious model that includes fewer number of variables.

Using the model selection criteria shown above, we can conduct stepwise model selection. There are mainly 3 ways- forward, backward and bidirectional selection.
...
Even though I excluded some variables, there are still _ variables so it needs to go further selection process to make more simpler, accurate model.
